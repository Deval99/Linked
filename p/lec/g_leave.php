...
if(isset($_SESSION['utype'])){
	if($_SESSION['utype'] == 'lecturer'){
		...
		if(!empty($le_id)){
			if(trim($le_id) != NULL){
				$query1 = "update `a_leave` set `granted?` = 1 where `le_id` = '$le_id' AND `l_id` = '$l_id' AND `granted?` = 0";
				if($run1 = $conn->query($query1)){
					if(mysqli_affected_rows($conn) != 0){
						echo "Grant Success ! -- Updated Rows = ".mysqli_affected_rows($conn);
					}else{
						echo "Nothing Updated !<br><br> Possible Reasons, <br><br>1.LeaveID is wrong.<br>2.Leave is already granted.<br>3.Student Belongs to another department !";
					}
				...
				header('Refresh:15; url= ../../form/lec/g_leave.php ',303,false);
			}
		}else{
