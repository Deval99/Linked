...
<body> <?php @session_start(); if(isset($_SESSION['utype'])){ if($_SESSION['utype'] == 'parent'){ ?>
  <form action='../p/a_leave.php' method='POST'>
  <h1>Ask For Leave</h1><br/>
  <h5>Enter Reason, *20 Charactors Allowed<br> For Faculty ID - *10 Charactors Allowed</h5>
  <span class='input'></span>
  <input type='text' required id='reason' maxlength="20" name='reason' placeholder='Reason' autofocus autocomplete='off' />

  <span class='input'></span>
  <input type='text' required id='l_id' maxlength="10" name='l_id' placeholder='Faculty ID (e.x. L01)' autofocus autocomplete='off' />
